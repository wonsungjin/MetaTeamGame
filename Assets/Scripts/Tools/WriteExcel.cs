...
namespace CreateScriptable
{
    internal class WriteExcel
    {
        ...
        public WriteExcel(List<string> data, string path, string GUID, string fileName)
        {
            Debug.Log("Start Write Asset File..");
            ...
            for (int i = 0; i < sw.Length; i++)
            {
                ...
                name[i] = splitDatas[i][1].Replace(" ","");
                for (int m = 0; m < splitDatas[i].Length; m++)
                    Debug.Log(splitDatas[i][m]);
